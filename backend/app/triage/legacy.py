...
8. General hard blockers — if the JD triggers any of criteria 2-4 above (a program-specific eligibility requirement he doesn't meet, e.g. "must be currently enrolled"; a staffing/contracting firm with an undisclosed end client; or a clearance/citizenship/license requirement), set "hard_blocker": true. These are genuine disqualifiers, not ordinary experience-gap mismatches, so they should route the same way a location hard blocker does — not sit in Review alongside roles that are merely a stretch or an imperfect fit.
...
Return ONLY valid JSON, no other text."""
        ...
        try:
            ...
            hard_blocker = bool(result.get("hard_blocker", False))
    ...
    for job, existing_reason in pursue:
        ...
        # A hard blocker — location (relocation required / non-NC residency
        # restriction / hybrid based outside the Triangle — see
        # candidate_profile.md's Location constraints section) OR general
        # (program-specific eligibility, staffing/contracting with an
        # undisclosed client, clearance/citizenship/license — criteria 2-4
        # above) is routed to Skipped (tagged "Skip:") instead: it's a harder
        # disqualifier than an ordinary demotion and shouldn't sit in Review.
        # Checked before the Stretch test since a hard blocker outranks an
        # experience-gap reason if Claude somehow flagged both. The engine
        # reads these prefixes to bucket the job. Threshold is config-driven.
        ...
        if location_hard_blocker or hard_blocker:
            _demote_prefix = "Skip"
            _dest = "Skipped"
        elif _is_stretch:
